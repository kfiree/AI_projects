...
# Create X and y variables for hypertension
# X = df.drop(['hypertension'],axis=1)
# y = df['hypertension'].to_frame()

# # Create X and y variables for heart_disease
# X = df.drop(['heart_disease'],axis=1)
# df.drop(['stroke'], axis=1, inplace=True);
# y = df['heart_disease'].to_frame()
...
df = orig_df
df.drop(['heart_disease'], axis = 1, inplace=True)
df.assign(heart_disease=heart_disease_new)
...
# Function to fit classifier and print accuracy score
def model(classifier, classifier_name, X_train = X_train_scaled, y_train = y_train,
                             X_test = X_test_scaled, y_test = y_test):
    # Fit classifier
    classifier.fit(X_train, np.ravel(y_train))
...
# # Fit Adaboost Classifier
model(adaboost_classifier, 'adaboost')

# # # Fit Decision trees Classifier
model(decision_trees_classifier, 'decision_trees')

# # Fit KNN Classifier
model(knn_classifier, 'knn')

# # Fir Logistic Regression Classifier
model(logistic_regression_classifier, 'logistic_regression')

for result in stroke_results:
    print(result, ': ', stroke_results[result]);
    print('--------------------------------')
